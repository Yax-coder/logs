...
import { useGetEventsLogQuery } from "./events.api";
...
const EventsLog = () => {
  ...
  const {
    ...
    isFetching,
    refetch,
  } = useGetEventsLogQuery(queryState);
  ...
  if (isLoading || isFetching) {
    return (
      <div>
        <div className="flex">
          <div className="flex-start mb-2 mr-2 w-1/3">
            <Skeleton count={1} height={50} />
          </div>
          <div className="mb-1 mr-2 w-1/4">
            <Skeleton count={1} height={50} />
          </div>
          <div className="mb-1 mr-2 w-1/4">
            <Skeleton count={1} height={50} />
  ...
  const handleOpen = (index) => {
    setCurrentItem(events.data[index]);
    setOpen(true);
  };

  const handleChange = (value) => {
    const { label, value: valueToRemove, ...newHeaders } = value;
    setQueryField("eventStatus", "");
    dispatch(setHeaders(newHeaders));
  };
  ...
  return (
    <div className="ml-10 mt-5">
        ...
        <div className="filters flex items-center justify-between gap-5">
          ...
          <div className="w-[300px]">
            <Select
              ...
              onChange={(val) => setQueryField("eventStatus", val.value)}
              valueAndLabel
            />
          </div>
        </div>
      ...
      <SideSheet width="70%" open={open} onClose={() => setOpen(false)}>
        ...
        >
          <h2 className="text-2xl">Event Detail</h2>
